                                              ...
                                              q7_t * bufferB)
{
    ...
    for (; i_out_y < dim_im_out_y - padding_y; i_out_y++)
    {
        ...
        for (i_out_x = 0; i_out_x < padding_x; i_out_x++)
        {
            ...
            if (pBuffer == bufferA + 2 * ch_im_in * dim_kernel_x * dim_kernel_y)
            {
                pOut = arm_nn_mat_mult_kernel_q7_q15_reordered(wt, bufferA, ch_im_out, ch_im_in * dim_kernel_x * dim_kernel_y,
                                                  bias_shift, out_shift, bias, pOut);
                /* counter reset */
                pBuffer = bufferA;
            }
...
arm_status arm_convolve_HWC_q7_fast_nonsquare_div(const q7_t * Im_in,
                                              const uint16_t dim_im_in_x,
                                              const uint16_t dim_im_in_y,
                                              const uint16_t ch_im_in,
                                              const q7_t * wt,
                                              ...
                                              q7_t * bufferB)
{
    ...
    for (i_out_y = 0; i_out_y < padding_y; i_out_y++)
    {
        for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++)
        {
            ...
            if (pBuffer == bufferA + 2 * ch_im_in * dim_kernel_x * dim_kernel_y)
            {
                pOut = arm_nn_mat_mult_kernel_q7_q15_reordered_div(wt, bufferA, ch_im_out, ch_im_in * dim_kernel_x * dim_kernel_y,
                                                  bias_shift, out_div, bias, pOut);
                /* counter reset */
                pBuffer = bufferA;
            }
        }
    ...
    for (; i_out_y < dim_im_out_y - padding_y; i_out_y++)
    {
        ...
        for (; i_out_x < dim_im_out_x - padding_x; i_out_x++)
        {
            ...
            {
                pOut =
                    arm_nn_mat_mult_kernel_q7_q15_reordered_div(wt, bufferA, ch_im_out, ch_im_in * dim_kernel_x * dim_kernel_y,
                                                  bias_shift, out_div, bias, pOut);
                /* counter reset */
                pBuffer = bufferA;
            }
