...
arm_status arm_convolve_HWC_q7_basic_nonsquare_div(	const q7_t * Im_in,
													const uint16_t dim_im_in_x,
													const uint16_t dim_im_in_y,
													const uint16_t ch_im_in,
													const q7_t * wt,
...
//    q31_t 	out_div_round =  ((out_div / 2) + 0.5 );
    ...
    for (i_out_y = 0; i_out_y < dim_im_out_y; i_out_y++)
    {
        for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++)
        {
            ...
            if (pBuffer == bufferA + 2 * ch_im_in * dim_kernel_y * dim_kernel_x)
            {
                pOut = arm_nn_mat_mult_kernel_q7_q15_div(wt, bufferA,
                                                  ch_im_out,
                                                  ch_im_in *
                                                  dim_kernel_y * dim_kernel_x, bias_shift, out_div, bias, pOut);
    ...
    if (pBuffer != bufferA)
    {
        ...
        {
            /* Load the accumulator with bias first */
//            q31_t     sum = ((q31_t)bias[i] << bias_shift) + out_div_round;
...
//            *pOut++ = (q7_t) __SSAT((sum / out_div) + 0.5, 8);
            tempOut = sum / out_div;
            if (tempOut < 0) *pOut++ = (q7_t) __SSAT(tempOut - 0.5, 8);
            else *pOut++ = (q7_t) __SSAT(tempOut + 0.5, 8);
        }
    }
#else
    /* Run the following code as reference implementation for Cortex-M0 and Cortex-M3 */
...
arm_status arm_convolve_HWC_q7_basic_nonsquare_o32(const q7_t * Im_in,
                                               const uint16_t dim_im_in_x,
                                               const uint16_t dim_im_in_y,
                                               const uint16_t ch_im_in,
                                               const q7_t * wt,
											   ...
											   int32_t * out_test)
{
    ...
    for (i_out_y = 0; i_out_y < dim_im_out_y; i_out_y++)
    {
        for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++)
        {
            for (i_ker_y = i_out_y * stride_y - padding_y; i_ker_y < i_out_y * stride_y - padding_y + dim_kernel_y; i_ker_y++)
            {
                for (i_ker_x = i_out_x * stride_x - padding_x; i_ker_x < i_out_x * stride_x - padding_x + dim_kernel_x; i_ker_x++)
                {
                    ...
                    } else
                    {
                        /* Copying the pixel data to column */
                        arm_q7_to_q15_no_shift((q7_t *)
                                               Im_in + (i_ker_y * dim_im_in_x + i_ker_x) * ch_im_in,
											   pBuffer,
											   ch_im_in);
                    }
                    pBuffer += ch_im_in;
            ...
            {
                tmp =
                    arm_nn_mat_mult_kernel_q7_q15_o32(	wt,
                    									bufferA,
														ch_im_out,
														ch_im_in * dim_kernel_y * dim_kernel_x,
														bias_shift,
														out_shift,
														bias,
														pOut,
