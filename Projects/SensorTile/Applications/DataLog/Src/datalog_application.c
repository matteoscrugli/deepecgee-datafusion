...
DrvStatusTypeDef getSensorsData( T_SensorsData *mptr)
{
  ...
  // if(!no_T_HTS221)
  // {
	if ( BSP_TEMPERATURE_Get_Temp( LPS22HB_T_0_handle, &mptr->temperature ) == COMPONENT_ERROR )	// NEW
...
DrvStatusTypeDef getGyroData( T_SensorsData *mptr)
{
  DrvStatusTypeDef ret = COMPONENT_OK;

  mptr->ms_counter = osKernelSysTick() - startingTime;
  ...
  mptr->genericX.AXIS_X = mptr->gyro.AXIS_X;
  mptr->genericX.AXIS_Y = mptr->gyro.AXIS_Y;
  mptr->genericX.AXIS_Z = mptr->gyro.AXIS_Z;

  return ret;
}


DrvStatusTypeDef getAccData( T_SensorsData *mptr)
{
  DrvStatusTypeDef ret = COMPONENT_OK;
  ...
  mptr->genericX.AXIS_X = mptr->acc.AXIS_X;
  mptr->genericX.AXIS_Y = mptr->acc.AXIS_Y;
  mptr->genericX.AXIS_Z = mptr->acc.AXIS_Z;

  return ret;
}


DrvStatusTypeDef getMagnData( T_SensorsData *mptr)
{
  DrvStatusTypeDef ret = COMPONENT_OK;
  ...
  mptr->genericX.AXIS_X = mptr->mag.AXIS_X;
  mptr->genericX.AXIS_Y = mptr->mag.AXIS_Y;
  mptr->genericX.AXIS_Z = mptr->mag.AXIS_Z;

  return ret;
}


DrvStatusTypeDef getPressData( T_SensorsData *mptr)
{
  DrvStatusTypeDef ret = COMPONENT_OK;
  ...
  mptr->generic = mptr->pressure;

  return ret;
}


DrvStatusTypeDef getTempData( T_SensorsData *mptr)
{
  DrvStatusTypeDef ret = COMPONENT_OK;
